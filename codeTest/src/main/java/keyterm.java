...
public class keyterm {
	...
	public void calPrw() throws IOException {
		...
		int index = 0;
		InputStream fr = keyterm.class.getClassLoader().getResourceAsStream(
				"all_prw.txt");
	...
	/*
	 * 将result_tmp中满足频率threshold的最长的串找出来，并且若其子串频率减去最长串频率还是大于threshold,将最长字串找出，
	 * 结果存入ht_result result_tmp是一个种子一个，ht_result是一篇文章一个
	 */
	public void getTerms(int threshold, Hashtable ht_all_subterms,
			Hashtable ht_result) {
		keyterm ck = new keyterm();
		...
		if (result_tmp != null) {
			...
			for (j = 0; j < result_tmp.length; j++) {
				if ((result_tmp[j] != null) && (result_tmp[j] != Max)) {
					int c1 = (new Integer(ht_all_subterms.get(Max).toString()))
							.intValue();
					int c2 = (new Integer(ht_all_subterms.get(result_tmp[j])
							.toString())).intValue();
			...
			if (contined_sub != null)// 只要满足频率的最长的子串
			{
				...
				if (MaxSub != null) {
					int c = (new Integer(ht_all_subterms.get(MaxSub).toString()))
							.intValue();
	 ...
	 */
	public void getAllTerms(Hashtable ht_result, List<String> ht_r,
			List<String> ht_n) {
		...
		for (Object key1 : ht_result.keySet()) {
			if (key1 != null) {
				if (!ht_r.contains(key1))// 如果list里面不包含这个词
				{
					...
					// ht_n[hr]=(new
					// Integer(ht_result.get(ht_r[hr]).toString())).intValue();
				...
				// 如果包含这个词
				{
					...
					if (loc != -1) {
						...
						int fresh = old
								+ (new Integer(ht_result.get(key1).toString()))
										.intValue();
						ht_n.get(loc).replace(ht_n.get(loc),
								new Integer(fresh).toString());
					}
		...
		for (int hh = 0; hh < ht_r.size(); hh++) {
			if (ht_r.get(hh) != null) {
				System.out.println("docManage中getAllTerms方法运行完后的结果为ht_r[j]"
						+ ht_r.get(hh));
	...
	public void docManage(String content, int threshold, List<String> ht_r,
			List<String> ht_n) {
		...
		{
			Hashtable ht_all_subterms = getAllSubs(threshold, i1.toString(),
					txt);// 获得包含种子i的所有子串
	...
	public Hashtable action(String filepath, int threshold, List<String> ht_r,
			List<String> ht_n) throws IOException {
		...
		String fl[] = { "", "", "" };// FileList.getFiles(filepath);//
										// 读入指定目录下的所有文件
		...
		for (j = 0; j < ht_r.size(); j++) {
			if (ht_r.get(j) != null) {
				for (int j1 = 0; j1 < stoplist.length; j1++) {
					String regex = "(" + stoplist[j1]
							+ "([\\u4e00-\\u9fa5]+))|(([\\u4e00-\\u9fa5]+)"
							+ stoplist[j1] + ")|(" + stoplist[j1]
							+ "([\\u4e00-\\u9fa5]+)" + stoplist[j1] + ")";
					// String
					// regex="(的(([\\u4e00-\\u9fa5]|[A-Za-z])+))|((([\\u4e00-\\u9fa5]|[A-Za-z])+)的)|(的(([\\u4e00-\\u9fa5]|[A-Za-z])+)的)";
					Pattern p = Pattern.compile(regex);
					Matcher m = p.matcher(ht_r.get(j));
					while (m.find()) {
						// String result=m.group();
						// System.out.println("result::"+result);
						if ((ht_r.get(j).charAt(ht_r.get(j).length() - 1)) == stoplist[j1]
								.charAt(0)) {
							ht_r.get(j).replace(
									ht_r.get(j),
									ht_r.get(j).substring(0,
											ht_r.get(j).length() - 1));
						}
						if (ht_r.get(j).charAt(0) == stoplist[j1].charAt(0)) {
							ht_r.get(j).replace(ht_r.get(j),
									ht_r.get(j).substring(1));
							// ht_r.get(j)=ht_r.get(j).substring(1);
						}
						if (((ht_r.get(j).charAt(ht_r.get(j).length() - 1)) == stoplist[j1]
								.charAt(0))
								&& (ht_r.get(j).charAt(0) == stoplist[j1]
										.charAt(0))) {
							ht_r.get(j).replace(
									ht_r.get(j),
									ht_r.get(j).substring(1,
					...
					// ht_n[j]=(new
					// Integer(all.get(ht_r[j]).toString())).intValue();
				}
			}
		}
		for (int hh = 0; hh < ht_r.size(); hh++) {
		...
		Arrays.sort(entries, new Comparator() {
			public int compare(Object arg0, Object arg1) {
				...
				int key2 = Integer.parseInt(((Map.Entry) arg1).getValue()
						.toString());
				return ((Comparable) new Integer(key1)).compareTo(new Integer(
						key2)); // ---当然为了降序排只要把key1和key2互换一下
			}
		});
		return entries;
	}
	...
	public Hashtable readDoc(String content) {// 将指定文本文件中每个字符在该文本的概率算出来
		...
		for (int i = 0; i < En_name.length; i++) {
			if (En_name[i] != null) { // System.out.println("英文单词为---"+En_name[i]);
				ht.put(En_name[i], (double) rd.getCount(s, En_name[i])
						/ (double) s.length());
				n++;
				// System.out.println(n);
				// System.out.println("英文单词为---"+En_name[i]+"---概率为---"+rd.getCount(s,
				// En_name[i])/s.length());
			}
		...
		for (int j = 0; j < Character.length; j++) {
			if (Character[j] != null) {
				ht.put(Character[j], (double) rd.getCount(s, Character[j])
						/ (double) s.length());
				n++;
				// System.out.println(n);
				// System.out.println("中文汉字为---"+Character[j]+"---概率为---"+(double)rd.getCount(s,
				// Character[j])/(double)s.length());
			}
		}
