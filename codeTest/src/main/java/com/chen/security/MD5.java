...
public class MD5
{
    ...
    private long[] count = new long[2]; // 存储原始信息的bits数长度,不包括填充的bits，最长为 2^64
                                        // bits
    ...
    /*
     * 对512bits信息(即block缓冲区)进行一次处理，每次处理包括四轮 state[4]:用于保存对512bits信息加密的中间结果或者最终结果
     ...
     * 将4字节的整数copy到字符形式的缓冲区中 output：用于输出的字符缓冲区 input：欲转换的四字节的整数形式的数组
    ...
    public static String byteHEX(byte b)
    {
        ...
        char[] ob = new char[2];
        ob[0] = Digit[(b >>> 4) & 0X0F]; // 左移四位使高四位和第四位调换，高四位置0，得十六进制的高位（>>>
                                         // 右移，左边空出的位以0填充）
    ...
    private void md5Update(byte[] inbuf, int inputLen)
    {
         ...
         * 用于判断已有信息加上当前传过来的信息的总长度能不能达到512bits， 如果能够达到则对凑够的512bits进行一次处理
         */
        ...
        if (inputLen >= partLen)
        {
            ...
            /*
             * 对当前输入的剩余字节做转换（如果剩余的字节<在输入的inbuft缓冲区中>大于512bits的话 ）， 转换结果保存到state中
             */
    ...
    private void md5Final()
    {
        ...
        // 信息的比特数右移3位即除以8，得到比特数，模64得到不足64一组的比特数
        ...
        // 判断最后一组的是否小于448比特（56字节），若大于则此组填512-index*8
        // 比特，下一组填充448比特，即共填充960-index*8 比特 ，即120-index 字节
    ...
    public static void main(String args[])
    {
        while (true)
        {
            ...
            System.out.println("Please input a string which you want to encode: ");
            System.out.println("The Encryption:  " + "\n" + md5.getMD5ofStr(sc.next()));
        }
    }
}